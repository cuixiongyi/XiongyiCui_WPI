...
int main()
{
	cv::Mat colorImg = cv::imread("valve1.jpg");
	//cv::Mat colorImg = cv::imread("testellipse.png");
	HoughEllipse he;
	//const int dims = 5;
	//int size[] = { 10000, 1, 1, 1, 1 };
	...
	int a;
	//std::cin >> a;
	cv::imshow("selectArea", colorImg);
	cv::setMouseCallback("selectArea", CallBackFunc);
	cv::waitKey(99999);
	cv::destroyWindow("selectArea");
	he.doHoughEllipse(colorImg(box));
	return 1;
}
...
//	IplImage *pEllipse;
//
//	pImgNoFilter = cvLoadImage("valve1.jpg");
//	pImg = cvLoadImage("valve1.jpg");
//	//sleep(1);
